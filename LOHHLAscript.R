...
option_list = list(
  ...
  make_option(c("-hlaLoc", "--HLAfastaLoc"), type="character", default='~/lohhla/data/hla_all.fasta', #!!! /ehpcdata/liao0101/CROPL-145_Neoantigen_180622/25_LOH/lohhla/data/example.patient.hlaFasta.fa
  ...
  make_option(c("-no", "--novoDir"), type="character", default='',                            #!!! /ehpcdata/wang_yujie/novocraft
              help="path to novoalign executable [default= %default]", metavar="character"),
  make_option(c("-ga", "--gatkDir"), type="character", default='',                            #!!! /ehpcdata/sge_software/GATK/3.5
              help="path to GATK executable [default= %default]", metavar="character"),
  make_option(c("-ex", "--HLAexonLoc"), type="character", default='~/lohhla/data/hla.dat',    #!!! /ehpcdata/liao0101/CROPL-145_Neoantigen_180622/25_LOH/lohhla/data/hla.dat
...
require(Biostrings, quietly = TRUE) ##Y
require(beeswarm, quietly = TRUE)
require(zoo, quietly = TRUE) ##Y
require(Rsamtools, quietly = TRUE) ##Y
...
)
{
  ...
  if(!override){
    ...
    for(BAM in BAMs){
      ...
      cmd    <- paste('samtools flagstat ', BAM, ' > ', outDir, region, '.proc.flagstat', sep = '') ##得到统计mappping信息的文本
      system(cmd)
    }  
  }    
...
if(mapping.step){
  ...
  patient.hlaFasta <- hlaFasta[hlaAlleles]  ##会出来两种HLA-A的参考序列放在一个fasta里吗？
  ...
  for(BAMfile in BAMfiles){
    ...
    ##把HLA基因区间内reads提取出来后再用novalign回帖到某人特异的HLA fasta上，然后排序，筛选，建index
    ...
    for (allele in hlaAlleles)
    {
      ...
      ##然后对bam文件进行筛选，去除have too many events，生成新的bam文件，并构建index  example_tumor_sorted.type.hla_a_01_01_01_01.filtered.bam
  }     ##我们自己的脚本已经完成这部分工作 制备某人特异的fasta，然后T N分开回帖做成bam，筛选，建index
...
for (region in regions)
{
  ...
  {      
    ...
    system(cmd)
    ##samtools mpileup example_tumor_sorted.type.hla_a_01_01_01_01.filtered.bam -f /ehpcdata/database_NGS/hg19/reference/HLA_db/abc_complete.fasta > example_tumor_sorted.hla_a_01_01_01_01.tumor.mpileup,生成统计了参考序列中每个碱基位点的比对情况的文本文件,每一行代表了参考序列中某一个碱基位点的比对结果
  }

}

# also extract number of unique reads sequenced in tumor and normal  ##处理对象是normal.bam,得到唯一位置回帖的reads的做法参考getUniqMapReads函数
if(runWithNormal){ ##应该是--normalBAMfile这个参数后面有输入
...
for (region in regions)
{
  ...
  if(runWithNormal){
    ...
    MultFactor      <- as.numeric(GermLineUniqMappedReads/UniqMappedReads)  ##参数M
  }   ##原始bam如何为uniq reads？
  ...
  {  
    ...
    for (HLA_gene in c('hla_a','hla_b','hla_c'))
    {
      ...
      HLA_A_type1 <- HLA_As[1]  ##单个HLA_a基因型
      HLA_A_type2 <- HLA_As[2]
      
      # the reference sequence for the patient's two HLA alleles
      HLA_type1Fasta <- hlaFasta[[HLA_A_type1]]  ##单个HLA_a的fasta
      ...
      #load the normal and tumor for each type
      #HLAtype 1 coverage
      {
        ...
        tmp <- intersect(rownames(HLA_A_type1tumor),rownames(HLA_A_type1normal)) ##得到重叠的rownames
        ...
        names(HLA_A_type1normalCov) <- HLA_A_type1normal$V2 #1:length(HLA_A_type1tumorCov) ##对每个reads数量精准定位
        
        HLA_A_type1tumorCov <- rep(0,length(HLA_A_type1normalCov))      
        names(HLA_A_type1tumorCov) <- names(HLA_A_type1normalCov)
        HLA_A_type1tumorCov[rownames(HLA_A_type1tumor)] <- HLA_A_type1tumor$V4  ##结果是一连串的对应有位置name的reads数字**
        
      }  ##HLA_A_type1tumor读取tumor.mpileup后用第二列位置信息作为列名，筛选出和normal位置重叠的部分
      ...
      if(extractNONmismatchReads%in%TRUE)
      {
        ...
        missMatchBed1    <- mismatchPosSeq1[,c(1,2,2)]        ##missMatchBed1是由提取了HLA_A_type1tumor第一列和两次第二列组成
        missMatchBed1$V2 <- as.numeric(missMatchBed1$V2)-1    ##并对第二列做了处理，变成4列
        missMatchBed1$V3 <- as.numeric(missMatchBed1$V2.1)+1  ##不明白这个V2.1是怎么冒出来的？奇怪的3变4
        ...
        ##这个missMatchBed是tumor.mpileup选取了存在于missMatchPositions$diffSeq1的位点，并截取部分列，调整数字得来的
        ...
        ##先生成missmatch bed，再bedtools intersect -v -a bam -b bed > NoMissMatch.bam
      ...
      missMatchseq2 <- missMatchPositions$diffSeq2[rowSums(misMatchCoveredInBoth)==2]  ##提取missMatchPositions$diffSeq1中满足上一条条件的部分
      ...
      dup1   <- unique(tmpOut_cn[duplicated(tmpOut_cn[,1]),1])  ##得到dup位点
      ...
      tmpOut_cn  <- tmpOut_cn[!duplicated(tmpOut_cn[,1]),,drop=FALSE]   ##不包括dup行
...
##目标参数 HLA_A_type1  HLA_A_type2  PVal_unique  LossAllele  KeptAllele  HLA_type1copyNum_withBAFBin  HLA_type2copyNum_withBAFBin    
##HLAtype1Log2MedianCoverage  HLAtype2Log2MedianCoverage  HLAtype1Log2MedianCoverageAtSites  HLAtype2Log2MedianCoverageAtSites  numMisMatchSitesCov UnPairedPval_unique
##HLA_type1copyNum_withBAFBin_lower  HLA_type1copyNum_withBAFBin_upper  HLA_type2copyNum_withBAFBin_lower  HLA_type2copyNum_withBAFBin_upper   propSupportiveSites 
...
##两个写出的表格：PatientOutPut**==out and combinedTable（全面）
